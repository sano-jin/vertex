...
extractPCtxName linkVal = error
                          $ "unexpected " ++ show linkVal
                          ++ ". Should not have collectly parsed." 


checkOpLinkVal :: PCtxEnv -> LinkVal -> ThrowsCompileError (PCtxEnv, Maybe Type)
checkOpLinkVal pCtxEnv pCtx@(ProcessContextVal name maybeType)
  = case M.lookup name pCtxEnv of
      Just maybeType' ->
        do unifiedType <- unifyType pCtx maybeType maybeType'
...
checkOpLinkVal pCtxEnv atomVal@(AtomVal atomName links@[l, r])
  = checkBinaryOp atomVal pCtxEnv l r
    =<< if isIntOp atomName then return $ Just TypeInt
        else throwError $ UnexpectedOpOnGuard atomVal
checkOpLinkVal pCtxEnv (DataVal (IntAtom _)) = return (pCtxEnv, Just TypeInt)
checkOpLinkVal pCtxEnv (DataVal (StringAtom _)) = return (pCtxEnv, Just TypeString)
checkOpLinkVal _ linkVal = throwError $ UnexpectedOpOnGuard linkVal


unifyType :: LinkVal -> Maybe Type -> Maybe Type -> ThrowsCompileError (Maybe Type)
unifyType linkVal lMaybeType rMaybeType
  = case (lMaybeType, rMaybeType) of
      (_, Nothing) -> return lMaybeType
      (Nothing, _) -> return rMaybeType
      (Just lType, Just rType)
        -> case (lType, rType) of
...
unifyEnvs linkVal leftPCtxEnv rightPCtxEnv
  = foldM addConstraint leftPCtxEnv $ M.toList rightPCtxEnv
  where
...
checkBinaryOp ::
  LinkVal -> PCtxEnv -> LinkVal -> LinkVal -> Maybe Type ->
  ThrowsCompileError (PCtxEnv, Maybe Type)
checkBinaryOp linkVal pCtxEnv  left right inferedType
  = do (leftPCtxEnv, leftMaybeType)   <- checkOpLinkVal pCtxEnv left
...
checkOpProcVal pCtxEnv (LocalAliasVal 0 _ linkVal@(AtomVal atomName links@[l, r]))
  = if atomName == ":=" then
      do leftPCtxName <- extractPCtxName l
         if M.member leftPCtxName pCtxEnv
           then throwError $ ShadowingProcessContext linkVal
           else (uncurry (M.insert leftPCtxName) . swap)
                <$> checkOpLinkVal pCtxEnv r
    else
      fst <$>
      ( checkBinaryOp linkVal pCtxEnv l r
        =<< if isPolymorphicOp atomName then return Nothing
            else if isIntOp atomName then return $ Just TypeInt
...
collectTypesLinkVal pCtxEnv pCtx@(ProcessContextVal name Nothing)
  = if M.member name pCtxEnv then return pCtxEnv
...
checkMultipleUtypedProcessContexts pCtxEnv
  = let untypedProcessContexts
          = S.fromList
          $ mapMaybe maybeProcessContextVal
          $ M.toList pCtxEnv in
      if S.size untypedProcessContexts > 1
      then throwError $ MultipleUtypedProcessContexts untypedProcessContexts
      else return pCtxEnv
  where maybeProcessContextVal (_, Just _) = Nothing
...
{--|
      inferedType <-            
        ...
        (leftPCtxEnv, leftMaybeType)   <- checkOpLinkVal pCtxEnv l
        (rightPCtxEnv, rightMaybeType) <- checkOpLinkVal pCtxEnv r
        unifiedPCtxEnv                 <-
          unifyEnvs linkVal leftPCtxEnv rightPCtxEnv
        unifiedType                    <-
          unifyType linkVal inferedType =<<
...
|--}
        ...
        {--|
checkOpProcVal pCtxEnv (LocalAliasVal 0 _ linkVal@(AtomVal atomName links@[l, r]))
  = if isGuardOp atomName then foldM checkOpLinkVal pCtxEnv links
    else if atomName == ":="
         then
           let leftPCtxName = extractPCtxName l in
             if M.member leftPCtxName pCtxEnv
             then throwError $ ShadowingProcessContext linkVal
             else checkOpLinkVal (M.insert atomName Nothing pCtxEnv) r
         else throwError $ UnexpectedOpOnGuard linkVal
|--}
