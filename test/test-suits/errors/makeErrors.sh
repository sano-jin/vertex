...
test unbound_process_context "a(\$p:int) :- b(\$q)"
test il_typed_process_context "a(\$p:string) :- \$p = 1 | b"
test il_typed_process_context "a(\$p:string) :- \$q = \$p + 1 | b(\$q)"
test il_typed_process_context "a(\$p:string), \$q:int :- \$p = \$q | b"
test multiple_untyped_process_contexts "a(\$p), \$q:- b"
test toplevel_process_context "a(\$p), \$q, b"
